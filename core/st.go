...
func Start(args string) {
	if args != "" {
		...
		_fullArgs := []string{os.Args[0]}
		os.Args = append(_fullArgs, _args...)
	}
...
func dnsServer() {
	...
	go func() {
		...
		dnsListenerServer = &dns.Server{Addr: dnsListen, Net: "udp"}
		err := dnsListenerServer.ListenAndServe()
		if err != nil {
